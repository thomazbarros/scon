...
import static org.mulgara.scon.impl.BooleanResultSet.Location.*;

/**
 * Contains the boolean results of a query operation.
 */
public class BooleanResultSet implements ResultSet {

  /** The parsed header data for this result set */
  private final ResultSetHeader header;

  /** The value of this result set. */
  private final boolean value;

  /** The statement used to genereate this result set. */
  private final Statement statement;

  /** A virtual location for a 1 row result */
  enum Location { BEFORE_FIRST, FIRST, AFTER_LAST };

  /** The current virtual location */
  private Location cursor = BEFORE_FIRST;

  /**
   * Creates this result set with a header and boolean value.
   */
  public BooleanResultSet(ResultSetHeader header, boolean value, Statement statement) {
    this.header = header;
    this.value = value;
  ...
  /**
   * Returns the type of this result set as being boolean.
   */
  ...
  /**
   * The boolean value for this result set.
   */
  ...
  public boolean absolute(int row) throws SparqlException {
    if (row <= 0) cursor = BEFORE_FIRST;
    else if (row == 1) cursor = FIRST;
    else cursor = AFTER_LAST;
    return cursor == FIRST;
  }
  ...
  public boolean relative(int rows) throws SparqlException {
    if (cursor == BEFORE_FIRST) {
      if (rows == 1) cursor = FIRST;
      else if (rows > 1) cursor = AFTER_LAST;
    } else if (cursor == FIRST) {
      if (rows < 0) cursor = BEFORE_FIRST;
      if (rows > 0) cursor = AFTER_LAST;
    } else {
      if (rows == -1) cursor = FIRST;
      else if (rows > 1) cursor = BEFORE_FIRST;
    }
    return cursor == FIRST;
  }
  ...
  public boolean next() throws SparqlException {
    switch (cursor) {
      ...
      case FIRST:
        ...
        break;
      case AFTER_LAST:
        throw new SparqlException("Beyond end of the set");
    }
    return cursor == FIRST;
  }
  ...
  public URI getUri(int column) throws SparqlException {
    throw new SparqlException("Boolean value is not a URI");
  }
  ...
  public ResultSetMetaData getMetaData() {
    return new BooleanMetaData(header);
  }
   ...
   */
  static class BooleanMetaData extends MetaData {

    BooleanMetaData(ResultSetHeader header) {
      super(header);
    }
